...
def test_guard_false():
    p = dotted.parse('enabled=False')
    assert dotted.assemble(p) == 'enabled=False'

def test_guard_wildcard_value():
    p = dotted.parse('*=*')
    assert dotted.assemble(p) == '*=*'

def test_neq_key_guard_assemble():
    p = dotted.parse('first!=7')
    assert dotted.assemble(p) == 'first!=7'

def test_neq_wildcard_guard_assemble():
    p = dotted.parse('*!=7')
    assert dotted.assemble(p) == '*!=7'

def test_neq_slot_guard_assemble():
    p = dotted.parse('[*]!=7')
    assert dotted.assemble(p) == '[*]!=7'
...
def test_continuation_wildcard():
    p = dotted.parse('a.*=7')
    assert dotted.assemble(p) == 'a.*=7'

def test_slot_continuation_assemble():
...
def test_dot_continuation_no_match():
    assert dotted.get({'a': {'first': 3}}, 'a.first=7') is None

def test_wildcard_continuation():
    d = {'a': {'val': 7}, 'b': {'val': 3}, 'c': {'val': 7}}
    r = dotted.get(d, 'a.*=7')
    assert r == (7,)

def test_slot_continuation():
    d = {'items': [10, 20, 30]}
...
def test_update_key_guard_match():
    d = {'a': 7, 'b': 3}
    r = dotted.update(d, 'a=7', 99)
    assert r == {'a': 99, 'b': 3}

def test_update_key_guard_no_match():
    d = {'a': 3, 'b': 7}
    r = dotted.update(d, 'a=7', 99)
    assert r == {'a': 3, 'b': 7}

def test_update_wildcard_guard():
    d = {'a': 7, 'b': 3, 'c': 7}
...
def test_remove_key_guard_match():
    d = {'a': 7, 'b': 3}
    r = dotted.remove(d, 'a=7')
    assert r == {'b': 3}

def test_remove_key_guard_no_match():
    d = {'a': 3, 'b': 7}
    r = dotted.remove(d, 'a=7')
    assert r == {'a': 3, 'b': 7}

def test_remove_wildcard_guard():
    d = {'a': 7, 'b': 3, 'c': 7}
    r = dotted.remove(d, '*=7')
    assert r == {'b': 3}
...
def test_pluck_key_guard():
    d = {'a': 7, 'b': 7, 'c': 3}
    r = dotted.pluck(d, '*=7')
    assert r == (('a', 7), ('b', 7))

def test_pluck_slot_guard():
...
def test_slicefilter_on_primitives_empty():
    """
...
def test_special_case_removed_consistency():
    """
...
def test_expand_wildcard_guard():
    d = {'a': 7, 'b': 3, 'c': 7}
    r = dotted.expand(d, '*=7')
    assert set(r) == {'a', 'c'}
